...
void NeuralNetwork::backward(const std::vector<double>& gradOutput) {
    std::vector<double> gradInput = gradOutput;

    // Iterate backward through the layers
    for (int i = layers.size() - 1; i >= 0; --i) {
        std::vector<double> currentGradInput(gradInput.size());
        std::vector<double> gradWeights;
        
        const std::vector<double>& currentLayerInput = (i == 0) ? gradOutput : layers[i-1]->getOutput();
        const std::vector<double>& currentLayerOutput = layers[i]->getOutput();

        layers[i]->backward(currentLayerInput, currentLayerOutput, gradInput, currentGradInput, gradWeights);
        
        gradInput = currentGradInput;
    }
}
